# Windows Installation Tool v8.0.1
...
> **2.** Scroll to the first "san_policy.xml" file and copy it line by line into lines 832-858. Put the code in between the `(echo ` and `)`. Remember to include all spaces!  
...
> **4.** Scroll to the second "san_policy.xml" file (Note that Microsoft calls it "san_policy.xml" on their website. It should actually be called "unattend.xml".) and copy it line by line into lines 872-892. Put the code in between the `(echo ` and `)`. Remember to include all spaces!  
