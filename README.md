# Windows Installation Tool v8.0.8
...
> **2.** Scroll to the first "san_policy.xml" file and copy it line by line into lines 827-853. Put the code in between the `(echo ` and `)`. Remember to include all spaces!  
...
> **4.** Scroll to the second "san_policy.xml" file (Note that Microsoft calls it "san_policy.xml" on their website. It should actually be called "unattend.xml".) and copy it line by line into lines 867-887. Put the code in between the `(echo ` and `)`. Remember to include all spaces!  
